      ...
      <el-form ref="userForm" :model="params">
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="code" :rules="[{ required: true, message: '请输入验证码', trigger: 'blur' }]">
          ...
          <div class="btn" style="position:absolute;right: 0;top: 6px;width: 40%;">
            <a href="javascript:" type="button" @click="getEmailCode()" :value="codeTest" style="border: none;background-color: none">{{codeTest}}</a>
          </div>
        </el-form-item>
    ...
    methods: {
      getEmailCode() {
        ...
        this.$refs.userForm.validateField('email', (errMsg) => {
          if (errMsg == '') {
            register.getEmailCode(this.params.email).then(res => {
              this.sending = false;
              this.timeDown();
            });
          }
      ...
      submitRegister() {
        this.$refs['userForm'].validate((valid) => {
          if (valid) {
            register.UserRegister(this.params).then(response => {
              //提示注册成功
              this.$message({
                type: 'success',
                message: "注册成功"
